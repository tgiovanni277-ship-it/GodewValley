...
# FIX 6: FARM_MIN y FARM_MAX ya no son constantes fijas —
#        level.gd los actualiza con set_farm_bounds() en _ready.

signal tool_used

const SPEED    = 80

# Límites del mapa — se sobreescriben desde level.gd
...
# FIX 6: llamado desde level.gd tras calcular el rect del mapa
func set_farm_bounds(min_pos: Vector2, max_pos: Vector2) -> void:
	farm_min = min_pos
	farm_max = max_pos
...
func _next_tool(dir: int) -> void:
	tool_index = (tool_index + dir) % TOOLS.size()
	if tool_index < 0:
		tool_index = TOOLS.size() - 1
